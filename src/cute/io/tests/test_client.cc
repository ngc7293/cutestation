...
TEST(Client, sends_commands_on_callback)
{
    ...
    *ios >> stream;
    ...
    EXPECT_EQ(packet.data().measurements(0).value_case(), cute::proto::Measurement::ValueCase::kState);
    EXPECT_EQ(packet.data().measurements(0).state(), 57);
}
...
TEST(Client, handles_sending_multiple_types)
{
    ...
    *ios >> stream;
    EXPECT_EQ(packet.data().measurements(0).state(), 87);
    *ios >> stream;
    EXPECT_EQ(packet.data().measurements(0).number(), 3.1415);
    *ios >> stream;
