...
TEST(TimeSeries, removes_data_older_than_length_before_now)
{
    ...
    now = 100;
    publisher.callPublish<double>("cute.data.test.double", 1.0, topic::time(100));
    EXPECT_EQ(series.size(), 1);

    now = 111;
    publisher.callPublish<double>("cute.data.test.double", 1.2, topic::time(111));
    EXPECT_EQ(series.size(), 1);
    now = 112;
    publisher.callPublish<double>("cute.data.test.double", 1.3, topic::time(112));
