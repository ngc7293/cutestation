...
SocketDispatcher::SocketDispatcher(MessageIngestor *ingestor)
  ...
  connect(server_, &QLocalServer::newConnection, this,
          &SocketDispatcher::openLocalConnection);
}
...
void SocketDispatcher::closeAll() { connectionClosed(); }

void SocketDispatcher::openLocalConnection() {
  SocketConnector *connector =
      new SocketConnector(server_->nextPendingConnection());
  connect(connector, &SocketConnector::messageReady, ingestor_,
          &MessageIngestor::receiveMessage);
  connect(this, &SocketDispatcher::connectionClosed, connector,
